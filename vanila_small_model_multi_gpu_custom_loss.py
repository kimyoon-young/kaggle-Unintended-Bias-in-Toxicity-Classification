...
bert_config = BertConfig(
    '../input/bert-pretrained-models/uncased_l-12_h-768_a-12/uncased_L-12_H-768_A-12/' + 'bert_config.json')
...
weights += (((train_only_df['target'].values >= 0.5).astype(bool).astype(np.int) +
             (train_only_df[identity_columns].fillna(0).values < 0.5).sum(axis=1).astype(bool).astype(
...
output_model_file = "small_bert_pytorch_170mi_300maxlen_lr2e5_batch64_custom_loss"
